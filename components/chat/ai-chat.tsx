...
> {
  ...
  componentDidUpdate(prev: { content: string }) {
    if (prev.content !== this.props.content && this.state.err)
...
export default function AIChatWidget() {
  ...
  useEffect(() => {
    articleCtxRef.current = articleCtx;
  }, [articleCtx]);
  ...
  const enqueue = useCallback(
    (chunk: string) => {
      ...
      if (chunkBufRef.current.length >= FLUSH_MIN_CHARS) {
        flushChunk();
        return;
      }
      if (flushTimerRef.current !== null) return;
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      await readSSE(res.body, (event, payload) => {
        if (event === "chunk") {
          ...
          const sources = (
            ...
            (s): s is MessageSource =>
              !!s &&
              typeof (s as MessageSource).slug === "string" &&
              typeof (s as MessageSource).title === "string",
          );
      ...
      if (!aborted) {
        const msg =
          err instanceof Error ? err.message : "AI 助手处理失败，请稍后重试";
        updateMsg((m) => ({
          ...m,
          content: `抱歉，出错了：${msg}`,
          isError: true,
        ...
        updateMsg((m) =>
          m.content ? m : { ...m, content: "已停止。", isError: true },
        );
      }
    } finally {
      abortRef.current = null;
