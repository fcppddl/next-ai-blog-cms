...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  const navItems = [
    ...
    { name: "分类管理", href: "/admin/categories", icon: FolderOpen, exact: false },
        ...
        <div className="h-full px-4 lg:px-6 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-2">
              <div className="w-9 h-9 bg-gray-900 dark:bg-gray-700 rounded-xl flex items-center justify-center shadow-sm">
                ...
                <div className="absolute right-0 top-11 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                  <div className="py-2">
                    <div className="px-4 py-2 border-b border-gray-100 dark:border-slate-700">
                      <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{displayName || session.user.username}</p>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.name} href={item.href} onClick={() => setSidebarOpen(false)}>
                ...
                >
                  <Icon className={cn("mr-3 h-4 w-4 flex-shrink-0", active ? "text-indigo-600 dark:text-violet-400" : "text-gray-400 dark:text-slate-500")} />
                  <span>{item.name}</span>
                  {active && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-indigo-400 dark:bg-violet-500" />}
                </div>
              </Link>
            );
          })}
