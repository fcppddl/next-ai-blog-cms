...
import { Save, X, Eye, EyeOff, Star, Tag, ArrowLeft, Link2, Folder, ImageIcon, AlignLeft } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { createSlug, calculateReadingTime, generateExcerpt } from "@/lib/utils";
import AIAssistant, { type AIRecommendation } from "@/components/admin/ai-assistant";
import TiptapEditor, { type TiptapEditorHandle } from "@/components/admin/tiptap-editor";
...
export default function PostEditor({ postId }: PostEditorProps) {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors } } =
    useForm<FormValues>({ defaultValues: { title: "", slug: "", excerpt: "", coverImage: "" } });
  ...
  const handleAIExcerptGenerated = (excerpt: string) => setValue("excerpt", excerpt);
  ...
  const handleAICategoryGenerated = (recommendation: AIRecommendation) => {
    const matched = categories.find((c) => recommendation.existing.includes(c.name));
  ...
  const onSubmit = useCallback(async (values: FormValues) => {
    ...
    setSaving(false);
  }, [content, published, featured, categoryId, selectedTags, isEditing, postId, router, toast]);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <Button type="button" variant="ghost" onClick={() => router.back()} className="gap-2 text-gray-500 hover:text-indigo-700 hover:bg-indigo-50 dark:text-slate-400 dark:hover:text-white dark:hover:bg-white/10">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Star className={cn("h-3.5 w-3.5", featured ? "fill-amber-500 text-amber-500 dark:fill-amber-400 dark:text-amber-400" : "")} />
            精选
          </button>
          <button
            type="button"
