...
| 对话上下文 | **`js-tiktoken`** | 估算 token，用于 companion 对话的上下文长度裁剪（默认 `cl100k_base`） |
...
下面内容与仓库根目录的 `.env.example` 保持一致，便于直接复制到你的 `.env.local` 后再按需调整。
...
# NextAuth 配置
NEXTAUTH_SECRET=""
# 必须带协议 http:// 或 https://，且与浏览器访问一致。若只写 IP:端口，NextAuth 会按 https 解析，纯 HTTP 访问时 Cookie 无法使用、登录后会话异常
# 本地: http://localhost:3000；直连 Node 端口: http://x.x.x.x:3000；若仅 Nginx 80 对外则不要带 :3000
NEXTAUTH_URL="http://localhost:3000"
# 线上若走 Nginx/Caddy 等反向代理，设为 true，NextAuth 会用 X-Forwarded-Host 识别真实域名（否则可能错误使用上面的 NEXTAUTH_URL）
...
# Embedding 配置 (用于向量生成)
EMBEDDING_API_KEY=""
# Embedding 接口地址（默认https://dashscope.aliyuncs.com/compatible-mode/v1）
...
# 可选：前台 AI 助手对话单次请求的上下文 token 上限（裁剪后的总预算），默认 8192
# COMPANION_MAX_CONTEXT_TOKENS=8192
```
...
# Embedding 配置（二选一）
# - 方案 A：使用 Ollama 生成向量
# - 方案 B：使用外部 Embedding 接口生成向量（见下方 EMBEDDING_*）
#
# 注意：OLLAMA_* 与 EMBEDDING_* 请二选一配置，否则向量生成来源不明确

# 方案 A：Ollama 配置 (用于向量生成)
OLLAMA_BASE_URL="http://localhost:11434"
OLLAMA_EMBEDDING_MODEL="nomic-embed-text"

# 方案 B：Embedding 配置 (用于向量生成)
# EMBEDDING_API_KEY="..."
# EMBEDDING_BASE_URL="https://dashscope.aliyuncs.com/compatible-mode/v1"
# EMBEDDING_MODEL="text-embedding-v4"
# EMBEDDING_DIMENSIONS="1024"
...
```bash
# 若你选择用 Ollama 生成向量（配置了 OLLAMA_*），需要启动并拉取嵌入模型
ollama serve
...
- **AI 流式响应**：写作与对话等路由使用 **`Content-Type: text/event-stream`（SSE）**，例如 `app/api/ai/write/` 下的生成与补全、`app/api/ai/companion/chat/stream`；对话摘要压缩为 **`POST`**：`app/api/ai/companion/context/compress`。
...
| 向量嵌入 | **二选一**：**Ollama**（`OLLAMA_*`）或外部 **Embedding 接口**（`EMBEDDING_*`）；由同一客户端上的 **`embed()`** 调用，**不经过**对话端点。 |
| 向量库 | **ChromaDB**（`CHROMADB_*`），见 `lib/vector/`。 |
| RAG 重排序（可选） | `lib/ai/rerank.ts`，环境变量 **`RAG_RERANK_*`**（见上文环境变量示例）。 |

### AI 助手对话上下文（压缩、裁剪）

前台 **AI 助手**（`components/chat/ai-chat.tsx`）与 **`/api/ai/companion/chat/stream`** 通过 **裁剪**（控制单次请求的 token）与 **压缩**（把过长历史收成摘要）配合，避免无界堆叠对话导致浪费与溢出。

### Context Engineering

本项目的 Context Engineering 主要覆盖四件事：

| 模块 | 作用 | 在本项目中的落点 |
|------|------|------------------|
| **知识路由（Knowledge Routing）** | 决定是否/如何走 RAG 检索与注入知识 | `lib/ai/knowledge-route.ts`、`app/api/ai/companion/knowledge/` 等 |
| **context 组装（Context Assembly）** | 把 system 指令、摘要、历史消息、本轮输入组装成一次请求上下文 | `lib/chat/context.ts`（`buildContext`、`ContextManager.prepareForRequest`） |
| **context 压缩（Context Compression）** | 生成/更新摘要（锚点记忆），把更早的历史折叠为更短的表示 | `app/api/ai/companion/context/compress`、`types/chat.ts`（`Message.summary`）、`lib/chat/` |
| **context 裁剪（Context Trimming）** | 在 token 预算内保留关键信息，避免上下文无界增长导致溢出与浪费 | `lib/chat/trimContext`（`js-tiktoken`，受 `COMPANION_MAX_CONTEXT_TOKENS` 控制） |

### Memory Architecture（三级）

本项目的 Memory Architecture 分为三级，分别解决不同尺度的信息保留与复用问题：

| 层级 | 作用 | 在本项目中的落点 |
|------|------|------------------|
| **会话级 memory** | 保留对话中的短期上下文与关键结论，避免上下文无限增长 | `Message.summary` 摘要锚点 + `buildContext` / `trimContext` + `context/compress`（见下表与 `lib/chat/`） |
| **知识库级 memory** | 将文章/文档切块向量化后检索，作为问答的可追溯事实来源 | ChromaDB 向量库 + RAG 检索（`lib/vector/`、`lib/ai/knowledge-route.ts` 等） |
| **系统级 memory** | 稳定的角色、人设、工具使用约束与策略（不随会话变化的“长期指令”） | system prompt（`lib/ai/prompts/` 等）与路由侧的 system 组装逻辑 |

| 项目 | 说明 |
|------|------|
| 类型 | `types/chat.ts` 中 **`Message.summary`**：记忆锚点；该条**之前**的内容在逻辑上已并入摘要。 |
| 组装 | `lib/chat/context.ts`：**`buildContext`**：主 system（人设 / RAG 等）→ 若有锚点则第二条 system 注入摘要 → **仅**锚点**之后**的消息；**`ContextManager.prepareForRequest`** 再追加本轮用户输入。 |
| **裁剪** | **`trimContext`**（`js-tiktoken`）：总 token 超过 **`COMPANION_MAX_CONTEXT_TOKENS`**（默认 8192）时，从近期段**头部**删起，保留主 system、摘要 system 与尾部本轮用户句。 |
| **压缩** | 锚点之后满 **5 轮（10 条）** 时，**`/api/ai/companion/context/compress`** 调用模型生成新摘要；输入为旧摘要 + 锚点后**紧邻 6 条**。成功后清空旧 `summary`，新摘要写在「旧锚点下标 + 6」对应消息上（**`compressMessages`**）。 |
| 前端 | 流式 **`done`** 后请求压缩接口；若 `compressed: true`，按返回列表更新，并按 **`id`** 合并旧消息以保留 **`sources` / `ragEnabled` / `ragUsed`**。 |

导出与常量见 **`lib/chat/index.ts`**（如 **`COMPRESS_MESSAGE_THRESHOLD`**、**`SUMMARY_ANCHOR_OFFSET_MESSAGES`**）。
...
app/                      # App Router：页面与 API
  ...
  api/
    auth/                 # NextAuth
    posts/ categories/ profile/ images/   # 公开 REST
    admin/                # 管理端 CRUD（需登录）
    ai/
      write/              # AI 写作（流式）
        generate/
        complete/
      companion/          # AI 助手对话（流式 + 记忆压缩）
        chat/
          stream/
        context/
          compress/
        knowledge/        # RAG 知识检索相关
        articles/         # 文章相关能力（若有）
components/               # admin、chat、layout、markdown、ui 等
lib/
  auth.ts prisma.ts utils.ts
  ai/                     # client、knowledge-route、prompts、rerank 等
  chat/                   # 对话上下文：buildContext、trimContext、ContextManager（见上文）
  vector/                 # chunker、Chroma 封装、indexer
  editor/                 # Tiptap AI 补全扩展
hooks/ types/             # 含 types/chat.ts（Message / CoreMessage）
