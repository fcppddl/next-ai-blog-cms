...
class ChromaVectorStore implements VectorStore {
  ...
  constructor() {
    const host = process.env.CHROMADB_HOST || process.env.CHROMA_HOST || "localhost";
    const port = parseInt(process.env.CHROMADB_PORT || process.env.CHROMA_PORT || "8000", 10);
  ...
  async search(queryVector: number[], options: SearchOptions = {}): Promise<SearchResult[]> {
    ...
    const results = await this.collection!.query({
      ...
      where: options.filters as Record<string, string | number | boolean> | undefined,
    });
