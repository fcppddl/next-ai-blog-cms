...
5. 在全部正文结束后，必须另起一行输出元数据（勿放入代码块、勿加反引号），且该行必须以 ${ASSISTANT_RAG_META_MARKER} 开头，后面紧跟合法 JSON，整行无其它前缀或后缀。格式示例：
${ASSISTANT_RAG_META_MARKER}{"ragUsed":true,"articles":[{"slug":"welcome-to-my-blog","title":"欢迎来到我的博客"}]}
字段说明：ragUsed 为 true 表示本回答实际依据了上方「检索到的相关内容」或「文章列表」中的具体文章；articles 只列出本回答中实际引用或依据的文章（slug、title 须与站内一致，最多 3 篇）。若未依据任何文章（例如纯闲聊、仅概括作者信息而未点名具体文章），ragUsed 为 false，articles 为 []。
JSON 必须是严格合法格式：布尔值为 true/false 英文小写，字符串内若含双引号须写成 \\" 转义，不要使用尾逗号，不要换行截断该行。`;
...
export function visibleStreamingPrefixLen(text: string, marker: string): number {
...
export function parseAssistantRagMeta(fullText: string): ParsedAssistantRagMeta {
  const marker = ASSISTANT_RAG_META_MARKER;
  const idx = fullText.lastIndexOf(marker);
  if (idx === -1) {
    return { displayText: fullText.trimEnd(), ragUsed: false, articles: [], parseOk: false };
  }
  const jsonPart = fullText.slice(idx + marker.length).trim();
  const displayText = fullText.slice(0, idx).trimEnd();
  try {
    const data = JSON.parse(jsonPart) as { ragUsed?: unknown; articles?: unknown };
    ...
    if (Array.isArray(data.articles)) {
      for (const a of data.articles) {
        ...
        if (!slug) continue;
        const title = typeof o.title === "string" && o.title.trim() ? o.title.trim() : slug;
        articles.push({ slug, title });
      }
    }
    return { displayText, ragUsed, articles: articles.slice(0, 5), parseOk: true };
  } catch {
    return { displayText: fullText.trimEnd(), ragUsed: false, articles: [], parseOk: false };
  }
}
